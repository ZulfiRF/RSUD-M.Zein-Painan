...
using System.Linq;
using Core.Framework.Windows.Attr;
using Core.Framework.Windows.Implementations.PropertyGrid.Implementation.Attributes;
            ...
            if (SelectedObject != null)
            {
                try
                {
                    ...
                    foreach (var descriptor in descriptors)
                    {
                        ...
                        if (isBrowsable)
                        {
                            var model = this.CreatePropertyItem(descriptor, propertyDef);
                            var property = SelectedObject.GetType().GetProperty(model.DescriptorDefinition.DisplayName);
                            if (!property.GetCustomAttributes(true).Any() || property.GetCustomAttributes(true).Any(n => !(n is NotViewInPropertyGridAttribute)))
                                propertyItems.Add(model);
                        }
                    }
                }
                catch (Exception e)
