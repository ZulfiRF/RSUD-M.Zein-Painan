...
namespace Core.Framework.Windows.Controls
{
    ...
    internal static class MetroWindowHelpers
    {
        ...
        public static void HandleWindowCommandsForFlyouts(this CoreUserControl window, IEnumerable<Flyout> flyouts, Brush resetBrush = null)
        {
            var flyout = flyouts
                .Where(x => x.IsOpen && (x.Position == Position.Right || x.Position == Position.Top))
                .OrderByDescending(Panel.GetZIndex)
        ...
        {
            if (window.WindowCommandsPresenter != null)
