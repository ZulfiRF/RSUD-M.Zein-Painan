...
using Core.Framework.Windows.Barcodes.client.result.optional;

namespace Core.Framework.Windows.Barcodes.client.result.optional
{
    ...
    sealed class NDEFSmartPosterResultParser : AbstractNDEFResultParser
    {
        public override ParsedResult parse(Result result)
        {
            ...
            sbyte[] signed = bytes.Select(b => (sbyte)b).ToArray();
            if (bytes == null)
            {
                return null;
            }
            var headerRecord = NDEFRecord.readRecord(signed, 0);
