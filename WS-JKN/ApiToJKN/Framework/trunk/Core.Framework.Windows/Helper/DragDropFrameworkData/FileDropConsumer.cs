namespace Core.Framework.Windows.Helper.DragDropFrameworkData
{
    ...
    public class FileDropConsumer : DataConsumerBase, IDataConsumer
    {
        ...
        {
            var files = this.GetData(e) as string[];
            if (files != null)
            {
                e.Effects = DragDropEffects.None;
                var dstItemsControl = sender as ItemsControl; // 'sender' is used when dropped in an empty area
                if (dstItemsControl != null)
                {
                    foreach (string file in files)
                    {
                        ...
                        else if (sender is ListBox)
                        {
                            ...
                            {
                                var dstItem =
                                    Utility.FindParentControlIncludingMe<ListBoxItem>(e.Source as DependencyObject);
