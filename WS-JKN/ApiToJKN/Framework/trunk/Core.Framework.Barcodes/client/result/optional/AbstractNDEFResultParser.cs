...
using System.Linq;
using System.Text;
using Core.Framework.Barcodes.Client.Result;

namespace Core.Framework.Barcodes.client.result.optional
{
   ...
   abstract class AbstractNDEFResultParser : ResultParser
   {
      ...
      internal static String bytesToString(sbyte[] bytes, int offset, int length, String encoding)
      {
         try
         {
            return Encoding.GetEncoding(encoding).GetString(bytes.Select(n=>Convert.ToByte(n)).ToArray(), offset, length);
         }
