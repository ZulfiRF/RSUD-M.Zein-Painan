...
using Core.Framework.Messaging.Handlers;
...
namespace Core.Framework.Messaging
{
    ...
    {
        private static readonly Thread[] ClientThreads = new Thread[Environment.ProcessorCount];
        ...
        static WebSocketServer()
        {
            ...
            CleanupThread = new Thread(HandleContextCleanupThread) { Name = "WebSocketServer Cleanup Thread" };
            CleanupThread.Start();

            for (int i = 0; i < ClientThreads.Length; i++)
            {
                ClientThreads[i] = new Thread(HandleClientThread) { Name = "WebSocketServer Client Thread #" + (i + 1) };
        ...
        private static void HandleContextCleanupThread()
        {
            while (!Cancellation.IsCancellationRequested)
            {
                ...
                var currentConnections = new List<Context>();
        ...
        {
            context.ReceiveEventArgs.UserToken = context;
            context.ReceiveEventArgs.Completed += ReceiveEventArgs_Completed;
            context.ReceiveEventArgs.SetBuffer(context.Buffer, 0, context.Buffer.Length);
        ...
        private void StartReceive(Context context)
        {
            try
            {
                if (context.ReceiveReady.Wait(TimeOut, Cancellation.Token))
                {
                    try
                    {
                        if (!context.Connection.Client.ReceiveAsync(context.ReceiveEventArgs))
                        {
                            ReceiveEventArgs_Completed(context.Connection.Client, context.ReceiveEventArgs);
                        }
                    }
        ...
        private void ReceiveEventArgs_Completed(object sender, SocketAsyncEventArgs e)
        {
            ...
            context.Reset();
            context.ReceivedByteCount = e.SocketError != SocketError.Success ? 0 : e.BytesTransferred;
