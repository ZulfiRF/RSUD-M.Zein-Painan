...
namespace Core.Framework.Messaging
{
    ...
    /// </summary>
    internal class AccessPolicyServer : TcpServer
    {
        /// <summary>
        ...
        private readonly string allowedHost = "localhost";
        private readonly int allowedPort = 80;
        ...
        public void SendResponse(TcpClient connection)
        {
            try
            {
                connection.Client.Send(
                    Encoding.UTF8.GetBytes(String.Format(Response, allowedHost, allowedPort.ToString(CultureInfo.InvariantCulture))));
            }
            catch (SocketException)
            {
                //Ignore
