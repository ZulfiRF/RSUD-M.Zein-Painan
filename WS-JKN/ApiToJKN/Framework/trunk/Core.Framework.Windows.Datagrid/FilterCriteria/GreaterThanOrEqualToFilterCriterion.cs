...
namespace Core.Framework.Windows.Datagrid.FilterCriteria
{
  [CriterionDescriptor( ">=@", FilterOperatorPrecedence.RelationalOperator, FilterTokenPriority.RelationalOperator )]
  [DebuggerDisplay( "GreaterThanOrEqualTo( {Value} )" )]
  public class GreaterThanOrEqualToFilterCriterion : RelationalFilterCriterion
  {
    public GreaterThanOrEqualToFilterCriterion()
      : base()
    {
    }

    public GreaterThanOrEqualToFilterCriterion( object value )
      : base( value )
    {
    }
    ...
    public override System.Linq.Expressions.Expression ToLinqExpression( System.Linq.IQueryable queryable, ParameterExpression parameterExpression, string propertyName )
    {
      ...
      return queryable.CreateGreaterThanOrEqualExpression( parameterExpression, propertyName, this.Value );
    }

    public override bool IsMatch( object value )
    {
      CompareResult result = this.Compare( value );
      return ( result == CompareResult.EqualsTo ) || ( result == CompareResult.GreaterThan );
    }

#if DEBUG
    public override string ToString()
    {
      StringBuilder stringBuilder = new StringBuilder( "GreaterThanOrEqualTo( " );
