...
using Core.Framework.Windows.Datagrid.Converters;
using Core.Framework.Windows.Datagrid.Utils.Math;
...
namespace Core.Framework.Windows.Datagrid
{
  ...
  public struct ColumnWidth
  {
    ...
    public ColumnWidth( double value, ColumnWidthUnitType type )
    {
      ...
      if( ( DoubleUtil.AreClose( value, 0d ) ) && ( type == ColumnWidthUnitType.Star ) )
    ...
    public static bool operator ==( ColumnWidth columnWidth1, ColumnWidth columnWidth2 )
    {
      return 
        ( ( columnWidth1.UnitType == columnWidth2.UnitType ) && 
          ( DoubleUtil.AreClose( columnWidth1.Value, columnWidth2.Value ) ) );
    }

    public static bool operator !=( ColumnWidth columnWidth1, ColumnWidth columnWidth2 )
    {
      if( columnWidth1.UnitType == columnWidth2.UnitType )
        return !DoubleUtil.AreClose( columnWidth1.Value, columnWidth2.Value );
