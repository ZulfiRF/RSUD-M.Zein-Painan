...
namespace ApiToJKN.Controllers
{
    public class AuthenticationController : ApiController
    {
        ...
        [HttpPost]
        public object Authentication([FromBody]AuthenticationRequest request)
        {
            object result;
            ...
            using (var db = new DbMedifirstDataContext(connection))
            {
                ...
                result = new AuthenticationMetaResponse()
                {
                    Metadata = new Metadata()
                    {
        ...
        [HttpPost]
        public object CheckDataSloting([FromBody] ChekSlotingReques request)
        {
            ...
            if (ConfigurationManager.AppSettings["polieksekutif"] != "1")
            {
                ...
                {
                    var error = new AuthenticationMetaResponse()
                    {
        ...
        [Route("GetAntrian")]
        [HttpPost]
        public object DaftarReservasi([FromBody] DaftarAntean request)
        {
        ...
        nextPoli:;
            ...
            nextStep2:
                ...
                {
                    //responEror result1;
                    var result1 = new AuthenticationMetaResponse()
                    {
            ...
            nextStep1:
                ...
                else
                {
                    ...
                    else if (dict["mr-nokartu"].ToString() != request.NoKartu.ToString())
                    {
                        var error = new AuthenticationMetaResponse()
                        {
                    ...
                    else if (Convert.ToInt32(DateTime.Now.Subtract(Convert.ToDateTime(dict["provperujuk-tglkunjungan"].ToString())).TotalDays) <= 90)
                    {
                        goto LolosRUjukan;
            ...
            LolosRUjukan:;
                ...
                if (request.NomorRm == "" || request.NomorRm == null)
                {
                    query3 = "select *  FROM         Reservasi INNER JOIN " +
                                " DetailReservasi ON Reservasi.KdRuangan = DetailReservasi.KdRuangan AND Reservasi.KodeBooking = DetailReservasi.KodeBooking " +
                                 " where DetailReservasi.noidentitas='" + request.Nik + "' and month(Reservasi.TglReservasi) ='" + Convert.ToDateTime(request.TanggalPeriksa).ToString("MM") + "' and  year(Reservasi.TglReservasi) ='" + Convert.ToDateTime(request.TanggalPeriksa).ToString("yyyy") + "' and  day(Reservasi.TglReservasi) ='" + Convert.ToDateTime(request.TanggalPeriksa).ToString("dd") + "'";
                }
                else
                {
                    query3 = "select *  FROM         Reservasi INNER JOIN " +
                               " DetailReservasi ON Reservasi.KdRuangan = DetailReservasi.KdRuangan AND Reservasi.KodeBooking = DetailReservasi.KodeBooking " +
                                " where DetailReservasi.nocm='" + request.NomorRm + "'and month(Reservasi.TglReservasi) ='" + Convert.ToDateTime(request.TanggalPeriksa).ToString("MM") + "' and  year(Reservasi.TglReservasi) ='" + Convert.ToDateTime(request.TanggalPeriksa).ToString("yyyy") + "' and  day(Reservasi.TglReservasi) ='" + Convert.ToDateTime(request.TanggalPeriksa).ToString("dd") + "'";
                ...
                if (isTrue.NoCm == null)
                {
                    ...
                    var query = "SELECT    ruangan.namaruangan, dbo.Jumlah_Pasien_Terdaftar('" + tglReservasi + "', " + kdKelompokWaktu + ", '" + request.KodePoli + "') AS JumlahPasienTerdaftar, Sloting.KdKelompokWaktu, Sloting.KdRuangan, Sloting.Slotting,  Sloting.Slotting - dbo.Jumlah_Pasien_Terdaftar('" + tglReservasi + "', " + kdKelompokWaktu + ", '" + request.KodePoli + "') AS SisaSloting FROM" +
                                " PasienDaftar INNER JOIN Ruangan ON PasienDaftar.KdRuanganAkhir = Ruangan.KdRuangan INNER JOIN Instalasi ON Ruangan.KdInstalasi = Instalasi.KdInstalasi INNER JOIN " +
                                " Sloting ON Ruangan.KdRuangan = Sloting.KdRuangan INNER JOIN MapingPeriodeWaktuAwalkeAkhir " +
                                " ON Sloting.KdKelompokWaktu = MapingPeriodeWaktuAwalkeAkhir.KdKelompokWaktu WHERE " +
                    ...
                    if (sloting != null)
                    {
                        ...
                        var query1 = "SELECT     Substring(Reservasi.NoReservasi,9, 3) as NoReservasi, Reservasi.KodeBooking, Reservasi.TglReservasi, Reservasi.TglRegistrasi, Reservasi.KdRuangan, Ruangan.NamaRuangan " +
                        ...
                        while (reader2.Read())
                        {
                            ...
                            antrean.estimasidilayani = Convert.ToString(Core.Framework.Helper.Date.DateHelper.ToInteger(Convert.ToDateTime(tanggalNow)));
        ...
        [HttpPost]
        public object CheckDataSlotingOprasi([FromBody] getJadwalOprasi request)
        {
            object result;
            var pasien = new PasienModel();
            DateTime LastUpdate = DateTime.Now;
            ...
            nextStep:
                ...
                else if (jadwal.KodeBooking == null)
                {
                    result = new AuthenticationMetaResponse()
                    {
                        Metadata = new Metadata()
        ...
        [HttpPost]
        public object CheckDataSlotingOprasiList([FromBody] getAllJadwalOprasi request)
        {
            object result;
            var pasien = new PasienModel();
            DateTime LastUpdate = DateTime.Now;
            ...
            nextStep:
                ...
                while (readerChekJadwal.Read())
                {
                    ...
                    jadwal.LastUpdate = Convert.ToInt64(Convert.ToString(Core.Framework.Helper.Date.DateHelper.ToInteger(DateTime.Now)));
        ...
        [HttpGet]
        public JsonResult<responEror> ResponErroSemua(string masagge)
        {
            try
            {
                responEror result;
        ...
        private string GetNoReservasi(DateTime? tglReservasi, string kdRuangan)
        {
            try
            {
