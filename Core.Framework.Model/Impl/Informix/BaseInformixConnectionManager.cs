...
namespace Core.Framework.Model.Impl.Informix
{
    ...
    /// </summary>
    public class BaseInformixConnectionManager : BaseConnectionManager
    {
        /// <summary>
        ///     Initializes a new instance of the <see cref="BaseConnectionManager" /> class.
        /// </summary>
        /// <param name="connectionString">The connection string.</param>
        public BaseInformixConnectionManager(string connectionString)
            : base(connectionString)
        {
            ConnectionString = connectionString;
        }

        public BaseInformixConnectionManager()
        {
        }
        ...
        {
            var query = "CREATE TABLE " + item.TableName + " ( \n";
            foreach (var field in item.Fields)
            {
                query += " " + field.FieldName + " " + MatchType(field) + " " +
                         ((field.IsAllowNull == SpesicicationType.AllowNull) ? " " : "NOT NULL") + ", \n";
            }
            query += "PRIMARY KEY   ( \n";
            ...
            query += ") Constraint  Pk_" + item.TableName + " ) ";
            ...
            using (var conn = Connection)
            {
                ...
                using (var command = Command)
                {
                    ...
                    try
                    {
                        ...
                        location = location + "\\" + item.TableName;
                        if (!Directory.Exists(location))
                        {
                            Directory.CreateDirectory(location);
                        }
                        File.WriteAllText(location + "\\" + item.TableName + ".sql", query);
        ...
        private string MatchType(FieldAttribute type)
        {
            if (type.Info.PropertyType.Name.Contains("String"))
            {
                if (type.Length == -1)
                    return "TEXT";
                if (type.Length > 254)
                    return "lvarchar (" + ((type.Length == null || type.Length == 0) ? "255" : type.Length.ToString()) +
                           ")";
                return "varchar (" + ((type.Length == null || type.Length == 0) ? "255" : type.Length.ToString()) +
                       ")";
            }
            if (type.Info.PropertyType.IsEnum || type.Info.PropertyType.Name.Contains("Char"))
            {
                return "varchar (1)";
            }
        ...
        public override ResultQuery CreateFilter(string input)
        {
            ...
            foreach (var query in input.Split(new[] {"and", "or"}, StringSplitOptions.RemoveEmptyEntries))
            {
                ...
                if (operate != 0)
                {
                    var operatCheck = input.Substring(operate - 3, 3);
